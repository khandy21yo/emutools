...
#
# This versionmaps the lat/long from a zipcode table to shoe the
# coverage of my current database.
#
...
while(@Fld = $sth->fetchrow_array())
{
	if ($Fld[0] != 0 && $Fld[1] != 0)
	{
		$im->setPixel(remapy($Fld[1]),remapx($Fld[0]),$blue);
	}
}
...
#
open(OUT, ">world_zipcode.png") || die;;
print OUT $im->png;
close OUT;
