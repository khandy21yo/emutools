...
//
// \note Uses mmap because it needs to have a look-ahead capability.
//   Accessing the source program using an array simplified other
//   aspects also.
//
// History
...
#include <sys/types.h>		// For mmap, open,fstat and friends
#include <sys/stat.h>		// ...
#include <sys/mman.h>		// ...
#include <unistd.h>		// ...
#include <fcntl.h>		// ...
...
void process(char *fn, bool debug)
{
	...
	{
		std::cerr << "Unable to fstat file " << fn << std::endl;
		return;
	}
	off_t length = statbuf.st_size;
	unsigned char *source;
	if ((source = (unsigned char *)mmap(0, length,
		PROT_READ, MAP_SHARED, fdin, 0)) == (void *)-1)
	{
		std::cerr << "Unable to mmap file " << fn << std::endl;
		return;
	}
	...
	while(keeplooping)
	{
		...
		case 29:	// Floating point constant
			{
				...
				buff[0] = source[offset++];
				buff[1] = source[offset++];
				buff[2] = source[offset++];
				buff[3] = source[offset++];
		...
		case 31:	// Double Floating point constant
				// \bug not always right?
			{
				...
				buff[2] = source[offset++];
				buff[3] = source[offset++];
				buff[4] = source[offset++];
				buff[5] = source[offset++];
				buff[6] = source[offset++];
				buff[7] = source[offset++];
		...
		case 58:	// Colon
				//
				// There is weirdness here because
				//  comments are handled weirdly.
				//  
			if ((source[offset] == 143) &&
				(source[offset + 1] == 217))
			{
				if (debug)
				{
...
void help_message()
{
	std::cerr << "ungwbasic -d -h <file>" << std::endl <<
